...
function PaginationItem({ type, number, content, onClick, disabled }) {
	let className =
		"inline-flex h-8 w-8 items-center justify-center rounded border border-gray-100 text-center text-sm";
	if (type === "current") {
		className =
			"inline-flex h-8 w-8 items-center justify-center rounded border-black bg-black text-white text-sm";
	}
...
function Pagination({ currentPage, lastPage, onPageChange }) {
	...
	return (
		<ol className="mt-8 flex flex-wrap justify-center gap-1 text-xs font-medium">
